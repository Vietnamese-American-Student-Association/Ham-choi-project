...
const GamesPage:React.FC = () => {
  ...
  const loadGames = async (name:string,h:'first'|'second')=>{
    ...
    // pre-populate winners map
    const map:Record<string,number> = {};
    data.forEach(g =>
      g.rounds.forEach(r =>{
        if (r.winnerId) map[`${g.id}-${r.index}`] = r.winnerId;
      })
    );
    setWinner(map);
    setLoading(false);
  ...
  useEffect(()=>{
    const n = sessionStorage.getItem('officerName');
    if (!n){ router.push('/login'); return; }
    setOfficer(n);
    loadGames(n,half);
  },[]);
  ...
  const report = async (
    ...
    // 1) update DB via RPC
    const res = await fetch('/api/reportGameResult',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({
        winning_team_id:win.id,
        losing_team_id :lose.id,
        game_id        :gId,
        round          :rd,
    ...
    await fetch('/api/logs',{
      ...
      body:JSON.stringify({
        ...
        gameName   :gName,
        round      :rd,
        winningTeam:win.name,
        losingTeam :lose.name
      })
    });
                    ...
                    {r.teams.map(t=>{
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...s.tBtn,
                            backgroundColor: cssColor(t.name),
                            opacity:wId && !isW ? 0.4 : 1
                          }}
                        >
                          {t.name}
                        </button>
